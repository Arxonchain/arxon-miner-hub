...
const Tasks = () => {
  ...
  // === New Video Task - https://youtu.be/dDfkFXB5pmE ===
  ...
  const availableRewards = tasks.reduce((sum, t) => sum + t.points_reward, 0) + 4000; // 3 hardcoded video tasks × 1000
        ...
        <ScrollReveal delay={0.1}>
          <div className="grid grid-cols-2 gap-3">
            <GlowCard glowColor="green" className="p-4 text-center">
              ...
              <p className="text-2xl font-bold text-foreground">{completedCount}/{tasks.length + 3}</p>
          ...
          <div className="space-y-3">
            ...
            <motion.div
              ...
              className={`p-4 rounded-xl border backdrop-blur-sm transition-all ${
                youtubeDone ? 'bg-green-500/5 border-green-500/30' : 'bg-red-500/5 border-red-500/20 hover:border-red-500/40 hover:bg-red-500/10'
              }`}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-foreground truncate">Subscribe to Arxon YouTube</h3>
                    {youtubeDone && (
                      <motion.span className="text-xs px-2 py-0.5 rounded-full bg-green-500/20 text-green-400 flex items-center gap-1">
                        <CheckCircle className="w-3 h-3" /> Done
                      </motion.span>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground mb-2">Subscribe to our official YouTube channel</p>
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-bold text-accent flex items-center gap-1">
                      <Zap className="w-3 h-3" /> +1,000 ARX-P
                    </span>
                ...
                <Button
                  size="sm"
                  onClick={completeYoutubeTask}
                  disabled={youtubeDone || completingYoutube}
                  className={youtubeDone ? 'bg-green-500/20 text-green-400 border border-green-500/30 cursor-default' : 'bg-red-600 text-white hover:bg-red-700'}
                >
                  {completingYoutube ? <Clock className="w-4 h-4 animate-spin" /> : youtubeDone ? <CheckCircle className="w-4 h-4" /> : 'Subscribe'}
                </Button>
              </div>
            </motion.div>

            {/* New Video Task - dDfkFXB5pmE */}
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.05 }}
              className={`p-4 rounded-xl border backdrop-blur-sm transition-all ${
                newVideoDone ? 'bg-green-500/5 border-green-500/30' : 'bg-red-500/5 border-red-500/20 hover:border-red-500/40 hover:bg-red-500/10'
              }`}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-foreground truncate">Watch Arxon Video</h3>
                    {newVideoDone && (
                      <motion.span className="text-xs px-2 py-0.5 rounded-full bg-green-500/20 text-green-400 flex items-center gap-1">
                        <CheckCircle className="w-3 h-3" /> Done
                      </motion.span>
                  ...
                  <p className="text-xs text-muted-foreground mb-2">Watch this important Arxon video and earn rewards</p>
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-bold text-accent flex items-center gap-1">
                      <Zap className="w-3 h-3" /> +1,000 ARX-P
                    </span>
                ...
                <Button
                  ...
                  className={newVideoDone ? 'bg-green-500/20 text-green-400 border border-green-500/30 cursor-default' : 'bg-red-600 text-white hover:bg-red-700'}
                >
                  {completingNewVideo ? <Clock className="w-4 h-4 animate-spin" /> : newVideoDone ? <CheckCircle className="w-4 h-4" /> : 'Watch Video'}
                </Button>
              </div>
            </motion.div>
            ...
            {loading ? (
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <motion.div key={i} className="h-24 rounded-xl bg-card/50 border border-border/30 animate-pulse" />
                ))}
              </div>
            ) : tasks.length === 0 ? (
                ...
                {tasks.map((task, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`p-4 rounded-xl border backdrop-blur-sm transition-all ${
                        isCompleted ? 'bg-green-500/5 border-green-500/30' : 'bg-card/50 border-border/40 hover:border-primary/30 hover:bg-card/60'
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3">
                            <span className="text-xs font-bold text-accent flex items-center gap-1">
                              <Zap className="w-3 h-3" /> +{task.points_reward} ARX-P
                            </span>
                            {task.external_url && (
                              <span className="text-xs text-muted-foreground flex items-center gap-1">
                        ...
                        <Button
                          ...
                          className={isCompleted ? 'bg-green-500/20 text-green-400 border border-green-500/30 cursor-default' : 'bg-primary text-primary-foreground hover:bg-primary/90'}
                        >
                          {isCompletingThis ? <Clock className="w-4 h-4 animate-spin" /> : isCompleted ? <CheckCircle className="w-4 h-4" /> : 'Claim'}
                        </Button>
                      </div>
                    </motion.div>
                  );
        ...
        <ScrollReveal delay={0.2}>
          <div className="glass-card p-4 border border-primary/20">
            <p className="text-xs text-muted-foreground text-center">
              Complete tasks to earn ARX-P points. Some tasks require external actions. Points are awarded immediately.
            </p>
          </div>
        </ScrollReveal>
      </main>
