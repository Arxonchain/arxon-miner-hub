 ...
 Deno.serve(async (req) => {
   ...
   try {
     ...
     // Parse CSV
     // Format: email,total_points,mining_points,task_points,social_points,referral_points,daily_streak,referral_bonus_pct,x_post_boost_pct,username,referral_code,signup_date
     ...
     for (let i = 0; i < lines.length; i++) {
       ...
       try {
         ...
         const { data: searchResult } = await supabaseAdmin
           .from('profiles')
           .select('user_id')
           .limit(1);
     ...
     return new Response(JSON.stringify({
       ...
       results,
       message: dryRun 
         ? `Preview: ${results.processed} users would be imported` 
         : `Imported ${results.created} users, skipped ${results.skipped} existing`
     }), {
       headers: { ...corsHeaders, 'Content-Type': 'application/json' },
     });
